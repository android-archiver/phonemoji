...
/**
 * Based on [android.telephony.PhoneNumberFormattingTextWatcher] with the following changes:
 ...
 */
class InternationalPhoneNumberFormattingTextWatcher : TextWatcher {
  ...
  ) : this(PhoneNumberUtil.createInstance(context), regionCode)

  @JvmOverloads constructor(phoneNumberUtil: PhoneNumberUtil, regionCode: String = Locale.getDefault().country) {
  ...
  @Synchronized override fun afterTextChanged(s: Editable) {
    ...
    TtsSpanHelper.addTtsSpan(phoneNumberUtil, s, 0, s.length)
  }
